...
def generate_timetable_pdf(entries, title, school_name, sub_title=""):
    ...
    for start, end in time_slots:
        ...
        for day in days:
            ...
            if match:
                # Show subject and either class or teacher depending on context
                content = f"<b>{match.get('subject_name', match.get('subjects', {}).get('name', 'N/A'))}</b>"
                if match.get("class_name"):
                    content += f"<br/>{match['class_name']}"
                elif match.get("classes", {}).get("name"):
                    content += f"<br/>{match['classes']['name']}"
                row.append(Paragraph(content, centered))
            else:
                row.append("")
        table_data.append(row)
        
    t = Table(table_data, colWidths=[35*mm] + [48*mm]*5)
